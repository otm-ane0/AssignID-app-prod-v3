...
interface WebOriginsTabProps {
  initialData?: WebOrigin[];
}
...
const defaultWebOrigins: WebOrigin[] = [
  {
    id: '1',
...
export default function WebOriginsTab({ initialData }: WebOriginsTabProps) {
  // Form state
  const [webOrigins, setWebOrigins] = useState<WebOrigin[]>(initialData || defaultWebOrigins);
  const [newOrigin, setNewOrigin] = useState('');
  
  // Track dirty state
  const [isDirty, setIsDirty] = useState(false);
  const [originalData, setOriginalData] = useState<WebOrigin[]>(initialData || defaultWebOrigins);
  ...
  useEffect(() => {
    const hasChanged = JSON.stringify(webOrigins) !== JSON.stringify(originalData);
    setIsDirty(hasChanged);
  }, [webOrigins, originalData]);
  ...
  const handleAddOrigin = () => {
    if (newOrigin.trim()) {
      const newWebOrigin: WebOrigin = {
        id: Date.now().toString(),
        origin: newOrigin.trim()
      };
      setWebOrigins([...webOrigins, newWebOrigin]);
      setNewOrigin('');
    }
  };

  const handleRemoveOrigin = (id: string) => {
    setWebOrigins(webOrigins.filter(origin => origin.id !== id));
  };

  const handleSave = async () => {
    setIsSaving(true);
    await new Promise(resolve => setTimeout(resolve, 600));
    setOriginalData(webOrigins);
    setIsDirty(false);
    setIsSaving(false);
    console.log('Saved:', webOrigins);
  };

  const handleDiscard = () => {
    setWebOrigins(originalData);
    setNewOrigin('');
    setIsDirty(false);
  };
        ...
        >
          ...
          <div style={{
            ...
            <div style={{
              ...
              <Network style={{ width: '24px', height: '24px', color: '#8b5cf6' }} />
            </div>
            <div>
              <h2 style={{
                fontSize: '20px',
              ...
              <p style={{
                ...
                Origins allowed to call endpoints from a browser app.
              </p>
            </div>
          </div>
            ...
            <div style={{ marginBottom: '32px' }}>
              <label style={{
                ...
                <div style={{
                  ...
                  <Link style={{ width: '12px', height: '12px', color: '#8b5cf6' }} />
                </div>
                Allowed web origin
              </label>
                ...
                <AnimatePresence mode="popLayout">
                  {webOrigins.map((item, index) => (
                    ...
                    >
                      ...
                      <motion.button
                        onClick={() => handleRemoveOrigin(item.id)}
            ...
            <div style={{
              ...
              <div style={{
                ...
                <motion.button
                  ...
                  whileHover={newOrigin.trim() ? { scale: 1.02 } : {}}
                  whileTap={newOrigin.trim() ? { scale: 0.98 } : {}}
                  style={{
                    height: '48px',
                    padding: '0 24px',
                    fontSize: '14px',
                    fontWeight: '600',
                    color: '#fff',
                    background: newOrigin.trim() 
                      ? 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)'
                      : '#94a3b8',
                    border: 'none',
                    borderRadius: '12px',
                    cursor: newOrigin.trim() ? 'pointer' : 'not-allowed',
                    transition: 'all 0.2s',
                    boxShadow: newOrigin.trim() ? '0 4px 12px rgba(59, 130, 246, 0.3)' : 'none',
                    opacity: newOrigin.trim() ? 1 : 0.6,
                    flexShrink: 0
                  }}
                  onMouseEnter={(e) => {
                    if (newOrigin.trim()) {
